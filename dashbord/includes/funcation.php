...
function view_notification_par($s_id)
{
    ...
    $today=date("Y/m/d");
    $fact = array();
    $query = "select * from notification where c_id={$s_id} and date='{$today}'";
    $find_class = mysqli_query($con, $query);
    if ($find_class)
    {
        while ($row = mysqli_fetch_assoc($find_class)) {
...
function sendabbcent($s_id)
{
    global $con;
    $today=date("Y/m/d");
    $query = "select * from attendance where s_id='{$s_id}' and date ='{$today}'";
    $find_att = mysqli_query($con, $query);
    $num_of_abs=mysqli_num_rows($find_att);
    if ($num_of_abs)
    {
        echo "<p>The student is absence today $today  </p><br>";
    }
    else {
        echo  "";
    }
