...
include  "includes/header.php";
        if (isset($_POST['submit']))
        {
           ...
           // $student_id =$_POST['student_id'];
            $status="parent";
            if (empty($id)or empty($fullname)or empty($phone)or empty($work_number)or empty($address)or empty($password))
            {
                ...
                else if ($count2)
                {
                    ...
                    $query = "INSERT INTO parent(p_id, p_name, phone_num,work_num,password,address,status)";
                    $query .= "VALUES({$id},'{$fullname}','{$phone}','{$work_number}','{$password}', '{$address}', '{$status}') ";
