        ...
        if (isset($_POST['submit']))
        {
            ...
            if (empty($id)or empty($fullname)or empty($phone)or empty($em_number)or empty($address)or empty($password)or empty($level) or empty($parent_id))
            {
                ...
                else if ($count2)
                {
                    ...
                    $query = "INSERT INTO student(s_id, s_name, s_phone, level,emg_num,password,address,class,status,p_id)";
                    $query .= "VALUES({$id},'{$fullname}','{$phone}','{$level}','{$em_number}','{$password}', '{$address}','0' ,'{$status}','{$parent_id}') ";
